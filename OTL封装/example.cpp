...
void MSSQL_TEST()
{
	...
	ezotl::ez_row_vec datav;
	// 准备插入数据
	// 方法 1  这个方法有问题, otl 在打开otl_stream 时, 如果sql是完整的, 会直接插入2次, 引起主键冲突的异常, 虽然也能插入成功,但是肯定会影响效率
	conn.exec("delete from OtlTypes");
	conn.insert(sql_direct_insert, datav);
	// 方法 2  完整的插入sql 应该使用这个方法
	conn.exec("delete from OtlTypes");
	conn.exec(sql_direct_insert);
	// 方法 3
	// ezotl::ezrowv vars;
	ezotl::ezvar().Set();
	otl_value();
